...
Examples:
  feat(api): add YooKassa checkout endpoint
  feat(worker): implement warmup ramp-up scheduler
  fix(worker): handle imapflow IMAP timeout on idle connection
  test(api): add integration tests for campaign send flow
  docs(roadmap): mark warmup-engine feature as done
  chore(db): add dns_* columns migration to email_accounts
  security(api): add YooKassa webhook HMAC verification
```
